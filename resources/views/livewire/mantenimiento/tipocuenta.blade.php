<div>
    ...
    @include('cruds.mantenimientos.tipocuenta.modal.modaltipocuenta')

    <div class="card-body">
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#createTipocuenta"> Crear un Tipo
            Cuenta</button>
    </div>
                        ...
                        @if ($data->isNotEmpty())
                            @foreach ($data as $p)
                                <tr>
                                    ...
                                    <td class="text-center "> {{ $p->signo }}</td>
                                    ...
                                    <td width="10px">
                                        <button class="btn btn-success" data-toggle="modal"
                                            data-target="#createTipocuenta"
                                            wire:click.prevent="editTipocuenta({{ $p->id }})">
                                            Editar
                                        </button>
                                    </td>
                                    <td width="10px">
                                        <button class="btn btn-danger"
                                            wire:click.prevent="$emit('eliminarRegistro','Seguro que deseas eliminar esta Cuenta?','eliminarTipocuenta', {{ $p->id }})">
                                            Eliminar
                                        </button>
                                    </td>
                                </tr>
